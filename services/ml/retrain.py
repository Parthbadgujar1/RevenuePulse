...
Model versioning:
  v3.1.0 → v3.1.1 → v3.1.2 → ... (patch increments on production retrains)
...
def load_retrain_state() -> dict:
    if os.path.exists(RETRAIN_STATE_PATH):
        with open(RETRAIN_STATE_PATH) as f:
            return json.load(f)
    return {"last_retrain_at": None, "last_retrain_rows": 0, "retrain_count": 0, "current_version": "baseline-recovery-v3.1.0"}
...
def bump_version(version: str) -> str:
    """Increment patch version: v3.1.0 → v3.1.1"""
...
def retrain(force: bool = False, dry_run: bool = False) -> dict:
    ...
    new_version = bump_version(state.get("current_version", "baseline-recovery-v3.1.0"))
